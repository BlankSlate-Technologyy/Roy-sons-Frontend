...
export const theme = {
  navy: "#1B365D", // Primary Deep Navy Shield
  navyDark: "#0F172A",
  green: "#16A34A", // Emerald Green Accent
  greenHover: "#15803D",
  navyLight: "#EDF2F7",
  white: "#FFFFFF",
  offWhite: "#F8FAFC",
...
export function MaxPakNavbar() {
  ...
  const isActive = (href) => {
    ...
    if (href === "/group-companies/max-pak-corrpration") {
      return pathname === "/group-companies/max-pak-corrpration" || pathname === "/group-companies/max-pak-corrpration/";
    }
      ...
      <div className="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <Link href="/group-companies/max-pak-corrpration" className="flex items-center gap-3 select-none group">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[9.5px] sm:text-[10.5px] font-bold tracking-[0.2em] uppercase" style={{ color: theme.green }}>
              Industrial Trading &amp; Supplies
            </span>
          </div>
        </Link>
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                key={link.label}
                href={link.href}
                className="text-[13px] font-bold tracking-wide uppercase transition-all duration-200 relative py-1 hover:text-[#1B365D] cursor-pointer"
                style={{
      ...
      <div className="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 pb-12 border-b" style={{ borderColor: theme.border }}>
          ...
          <div className="lg:col-span-4">
            <Link href="/group-companies/max-pak-corrpration" className="flex items-center gap-3.5 mb-5 select-none">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold tracking-[0.2em] uppercase" style={{ color: theme.green }}>
                  Industrial Trading &amp; Supplies
                </p>
              </div>
            </Link>
            ...
            <p className="text-xs sm:text-sm font-medium leading-relaxed mb-6" style={{ color: theme.textMuted }}>
              Powering industries with excellence. A premier industrial trading, heavy machinery, construction materials, and commercial supplies enterprise delivering turnkey engineering procurement to infrastructure and manufacturing projects across Pakistan.
            </p>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-full border text-[11px] font-bold bg-slate-50" style={{ borderColor: theme.border, color: theme.navyDark }}>
                <Package size={14} style={{ color: theme.navy }} />
            ...
            <ul className="space-y-2.5 text-xs font-semibold">
              {NAV_LINKS.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="hover:underline transition-colors hover:text-[#1B365D] flex items-center gap-1.5"
                    style={{ color: theme.textMuted }}
                  >
                    <ChevronRight size={12} style={{ color: theme.green }} />
                    <span>{link.label}</span>
                  </Link>
                </li>
              ))}
          ...
          <div className="lg:col-span-3">
            ...
            <div className="mt-5 p-3.5 rounded-xl border bg-slate-50 flex items-center gap-3" style={{ borderColor: theme.border }}>
              ...
              <div>
                <p className="text-[11px] font-bold uppercase" style={{ color: theme.navyDark }}>Industrial RFQ Desk</p>
                <p className="text-[10.5px] font-medium" style={{ color: theme.textMuted }}>24/7 Turnkey procurement support</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-medium" style={{ color: theme.textLight }}>
          <p>&copy; 2026 Max Pak Corporation. A Roy &amp; Sons Group Company.</p>
          <div className="flex items-center gap-4">
            <Link href="/group-companies/max-pak-corrpration/contact" className="hover:underline hover:text-[#1B365D]">
              ISO 9001:2015 Certified Supply Chain
            </Link>
            <span>•</span>
            <Link href="/group-companies/max-pak-corrpration/contact" className="hover:underline hover:text-[#1B365D]">
              ASTM &amp; DIN Compliant Industrial Products
            </Link>
          </div>
        </div>
      </div>
